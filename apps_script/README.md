...
這份指引帶你把 `apps_script/` 裡的 7 個檔案部署到 Google Apps Script，從零到每天 08:00 自動跑完整 Pipeline。
...
## Step 7：設定每日 08:00 自動觸發

1. 函式下拉選 `setupDailyTrigger`
2. 按 **執行**
3. 完成後左側「觸發條件」圖示（時鐘 ⏰）會看到一筆每天 08:00 的觸發
...
### 改 Prompt / 參數
- 直接編輯對應的 .gs 檔，存檔即生效，下次 08:00 自動套用
